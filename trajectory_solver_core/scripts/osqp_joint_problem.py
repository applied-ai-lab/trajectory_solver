...
from trajectory_solver.JointSpaceProblem import JointSpaceProblem, HandEnum, JointAngles
...
def test():
    ...
    dof = 7
    pose_problem = JointSpaceProblem(namespace, spline_N, True)

    pose_problem.create(verbose=True)

    pose_problem.initialise()

    poses = {HandEnum.LEFT: JointAngles(dof), 
             HandEnum.RIGHT: JointAngles(dof)}

    poses[HandEnum.RIGHT].init_q.pos = 3.1 * np.ones(dof)
    poses[HandEnum.LEFT].init_q.pos = 3.1 * np.ones(dof)

    poses[HandEnum.RIGHT].init_q.vel = 0.15 * np.ones(dof)
    poses[HandEnum.LEFT].init_q.vel = 0.15 * np.ones(dof)

    poses[HandEnum.RIGHT].mid_q.pos = -0.15 * np.ones(dof)
    poses[HandEnum.LEFT].mid_q.pos = -0.15 * np.ones(dof)

    poses[HandEnum.RIGHT].end_q.pos = 1.1 * np.ones(dof)
    poses[HandEnum.LEFT].end_q.pos = 1.1 * np.ones(dof)
